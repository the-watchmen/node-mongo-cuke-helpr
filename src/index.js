...
import {asTemplate, evalInContext, setState, getRequiredState} from '@watchmen/test-helpr'
import {getDb, createIndices, createValidator} from '@watchmen/mongo-helpr'
import {stringify, diffConsole, isLike, isLikeHooks} from '@watchmen/helpr'
...
export default function(context) {
  defineSupportCode(({Given, When, Then}) => {
    Given('the following documents exist in the {string} collection:', async function(
    ...
    Given('the following indices exist on the {string} collection:', async function(
      collectionNameString,
      indicesString
    ) {
      try {
    ...
    Given('the following validator exists on the {string} collection:', async function(
      collectionNameString,
      validatorString
    ) {
      try {
    ...
    Then('mongo query {string} on {string} should be like:', async function(
      queryString,
      collectionNameString,
      expectedString
    ) {
    ...
    When('the following aggregation steps execute on the {string} collection:', async function(
      collectionNameString,
      stepsString
    ) {
      const collectionName = evalInContext({js: asTemplate(collectionNameString), context})
      ...
      dbg(
        'when-aggregation-executes-on: collection=%o, steps=%s',
        collectionName,
    ...
    Then('the result should be like:', async function(expectedString) {
      const expected = evalInContext({js: expectedString, context})
